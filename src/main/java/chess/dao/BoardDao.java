...
/**
 * BoardDao 클래스는 DB와의 통신을 위한 클래스로, DB에 보드에 있는 말들의 기록을 저장하거나 불러오는 작업을 수행한다.
 * 필요한 테이블 조작 기능 두 가지를 메서드로 가지고 있다.
 * 기능의 목록은 다음과 같다.
 * - update : 기존의 기록을 대체하는 새로운 기록을 테이블에 입력한다. 각 줄을 문자열로 변환한 값을, Column을 키로 저장한다.
 * - select : 테이블의 모든 기록을 가져온다. 불러오기 기능에서 사용된다.
 */
