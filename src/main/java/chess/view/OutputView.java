...
public final class OutputView {
    ...
    public static void printInitialRoomMessage(List<Integer> ids) {
        String rooms = ids.stream().map(value -> Integer.toString(value)).collect(Collectors.joining(", "));
        System.out.println("> 체스 게임에 오신 것을 환영합니다.\n"
                + "> 게임방 생성 : create\n"
                + "> 게임방 불러오기 : load 방번호 - 예. load 2\n"
                + "> 현재 존재하는 게임방 id: " + rooms);
    }

    public static void printRoomEntryMessage(final int id) {
        System.out.println("> " + id + "번 방에 입장하셨습니다.");
    }
