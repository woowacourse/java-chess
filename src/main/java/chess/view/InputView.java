...
public class InputView {
    ...
    private static List<String> checkCommand(String input) {
        List<String> splitInput = Arrays.asList(input.split(BLANK));
        String command = splitInput.get(COMMAND_INDEX);
        if (isEnd(splitInput, command) || isMove(splitInput, command) || isStatus(splitInput, command)) {
            return splitInput;
        }
    ...
    private static boolean isStatus(List<String> splitInput, String command) {
        return command.equals(STATUS_COMMAND) && splitInput.size() == STATUS_COMMAND_SIZE;
    }
