...
public enum PieceConverter {
	...
	PieceConverter(char name, Function<String, ChessPiece> creator) {
		this.name = name;
		this.creator = creator;
	}

	public static ChessPiece convert(char pieceName, String position) {
		return Arrays.stream(values())
			.filter(converter -> converter.name == pieceName)
			.findFirst()
