package chess.domain;

import java.util.*;
import java.util.stream.Collectors;

public class Knight extends Piece {

    private final static Map<String, Knight> CACHE = new HashMap<>();

    static {
        for (Color color : Color.values()) {
            CACHE.put(color.getName(), new Knight(color));
        }
    }

    private final Type type = Type.KNIGHT;

    public Knight(Color color) {
        super(color);
    }

    public static Knight of(Color color) {
        validateInput(color);
        return CACHE.get(color.getName());
    }

    private static void validateInput(Color color) {
        if (Objects.isNull(color)) {
            throw new IllegalArgumentException("잘못된 입력입니다");
        }
    }

    @Override
    public Set<Square> calculateScope(Square square) {
        Set<Square> availableSquares = new HashSet<>();
        int x = -1;
        int y = 2;
        for (int i = 0; i < 2; i++) {
            availableSquares.add(getMovedSquareByFileAndRank(square, x, y));
            availableSquares.add(getMovedSquareByFileAndRank(square, y, x));
            availableSquares.add(getMovedSquareByFileAndRank(square, x, (-1) * y));
            availableSquares.add(getMovedSquareByFileAndRank(square, y * -1, x));
            x *= -1;
            y *= -1;
        }
        return availableSquares;
    }

    @Override
    public Set<Square> calculateMoveBoundary(Square square, Map<Square, Piece> board) {
        validateNotNull(square, board);
        return calculateScope(square).stream()
                .filter(knightScopeSquare ->
                        !(board.containsKey(knightScopeSquare) && board.get(knightScopeSquare).color == color))
                .collect(Collectors.toSet());
    }

    @Override
    public double getScore() {
        return type.getScore();
    }

    @Override
    public String getLetter() {
        if (color == Color.BLACK) {
            return type.getName();
        }
        return type.getName().toLowerCase();
    }
}
