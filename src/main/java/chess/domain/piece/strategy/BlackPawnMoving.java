...
public class BlackPawnMoving implements Movable {

    private static final int INITIAL_PAWN_COORDINATE_Y = 7;
    ...
    private boolean isDiagonal(final Position from, final Position to) {
        ...
        return Direction.blackPawnDiagonalStep().contains(direction);
    }

    private boolean isForward(final Position from, final Position to) {
        Direction direction = Direction.getDirection(from, to);
        if (isStarting(from)) {
            return Direction.blackPawnStartingForwardStep().contains(direction);
        }
        return Direction.blackPawnForwardStep().contains(direction);
    }
