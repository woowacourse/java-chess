...
public class WhitePiecesFactory {
	...
	private static void initWhiteKing(Map<Position, Piece> pieces) {
		pieces.put(Position.of("e1"), new King(new Initial(), "k"));
	}

	private static void initWhiteQueen(Map<Position, Piece> pieces) {
		pieces.put(Position.of("d1"), new Queen(new Initial(), "q"));
	}

	private static void initWhiteKnight(Map<Position, Piece> pieces) {
		pieces.put(Position.of("b1"), new Knight(new Initial(), "n"));
		pieces.put(Position.of("g1"), new Knight(new Initial(), "n"));
	}

	private static void initWhiteBishop(Map<Position, Piece> pieces) {
		pieces.put(Position.of("c1"), new Bishop(new Initial(), "b"));
		pieces.put(Position.of("f1"), new Bishop(new Initial(), "b"));
	}

	private static void initWhiteRook(Map<Position, Piece> pieces) {
		pieces.put(Position.of("a1"), new Rook(new Initial(), "r"));
		pieces.put(Position.of("h1"), new Rook(new Initial(), "r"));
	}

	private static void initWhitePawn(Map<Position, Piece> pieces) {
		for (File file : File.values()) {
			pieces.put(Position.of(file.getFile() + "2"), new Pawn(new Initial(), "p"));
		}
	}
}
