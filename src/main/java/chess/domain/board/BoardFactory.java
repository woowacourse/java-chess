...
public class BoardFactory {
    private static final Map<Team, Map<Role, Rank>> INITIAL_RANKS;
    ...
    private static Map<Team, Map<Role, Rank>> createInitialRanks() {
        Map<Team, Map<Role, Rank>> initialRanks = new HashMap<>();
        ...
        initialRanks.put(Team.from(Color.BLACK), blackRanks);
        initialRanks.put(Team.from(Color.WHITE), whiteRanks);
        return initialRanks;
    }

    public static Board create() {
        setup(Team.from(Color.BLACK));
        setup(Team.from(Color.WHITE));
    ...
    private static void setup(final Team team) {
        Map<Role, Rank> ranksBySide = INITIAL_RANKS.get(team);
        for (Role role : INITIAL_FILES.keySet()) {
            putPiece(ranksBySide.get(role), INITIAL_FILES.get(role), role.create(team));
        }
    }
