// chessboard.js v1.0.0 https://github.com/oakmac/chessboardjs/
//
// Copyright (c) 2019, Chris Oakman Released under the MIT license
// https://github.com/oakmac/chessboardjs/blob/master/LICENSE.md start anonymous
// scope;
(function () {
    'use strict'
    ...
    // use unique class names to prevent clashing with anything else on the page and
    // simplify selectors
    ...
    // function debounce (f, interval, scope) {   var timeout = 0   return function
    // (_args) {     window.clearTimeout(timeout)     var args = arguments timeout =
    // window.setTimeout(function () {       f.apply(scope, args)     }, interval)
    // } }
    ...
    // convert FEN string to position object returns false if the FEN string is
    // invalid
    ...
    // position object to FEN string returns false if the obj is not a valid
    // position object
    ...
    // returns the square of the closest instance of piece returns false if no
    // instance of piece is found in position
    ...
    // given a position and a set of moves, return a new position with the moves
    // executed
    ...
    function checkJQuery() {
        if (!validJQueryVersion()) {
            var errorMsg = 'Chessboard Error 1005: Unable to find a valid version of jQuery. Please include ' +
                    'jQuery ' + MINIMUM_JQUERY_VERSION + ' or higher on the page\n\nExiting' + ELLIPSIS
    ...
    function checkContainerArg(containerElOrString) {
        if (containerElOrString === '') {
            var errorMsg1 = 'Chessboard Error 1001: The first argument to Chessboard() cannot be an empty str' +
                    'ing.\n\nExiting' + ELLIPSIS
        ...
        if ($container.length !== 1) {
            var errorMsg2 = 'Chessboard Error 1003: The first argument to Chessboard() must be the ID of a DO' +
                    'M node, an ID query selector, or a single DOM node.\n\nExiting' + ELLIPSIS
    ...
    function constructor(containerElOrString, config) {
        ...
        // Misc
        // -------------------------------------------------------------------------
        // calculates square size based on the width of the container got a little CSS
        // black magic here, so let me explain: get the width of the container element
        // (could be anything), reduce by 1 for fudge factor, and then keep reducing
        // until we find an exact mod 8 for our square size
        ...
        function animateSquareToSquare(src, dest, piece, completeFn) {
            ...
            // create the animated piece and absolutely position it over the source square
            var animatedPieceId = uuid()
            $('body').append(buildPieceHTML(piece, true, animatedPieceId))
            var $animatedPiece = $('#' + animatedPieceId)
            $animatedPiece.css({display: '', position: 'absolute', top: srcSquarePosition.top, left: srcSquarePosition.left})
        ...
        function animateSparePieceToSquare(piece, dest, completeFn) {
            ...
            $animatedPiece.css({display: '', position: 'absolute', left: srcOffset.left, top: srcOffset.top})
        ...
        // calculate an array of animations that need to happen in order to get from
        // pos1 to pos2
        function calculateAnimations(pos1, pos2) {
            ...
            for (i in pos2) {
                ...
                if (closestPiece) {
                    animations.push({type: 'move', source: closestPiece, destination: i, piece: pos2[i]})
            ...
            for (i in pos2) {
                ...
                animations.push({type: 'add', square: i, piece: pos2[i]})
            ...
            for (i in pos1) {
                ...
                animations.push({type: 'clear', square: i, piece: pos1[i]})
        ...
        function beginDraggingPiece(source, piece, x, y) {
            // run their custom onDragStart function their custom onDragStart function can
            // cancel drag start
            ...
            $draggedPiece
                .attr('src', buildPieceImgSrc(piece))
                .css({
                    display: '',
                    position: 'absolute',
                    left: x - squareSize / 2,
        ...
        async function stopDraggedPiece(location) {
            ...
            if (isFunction(config.onDrop)) {
                ...
                // source piece is a spare piece and position is off the board if
                // (draggedPieceSource === 'spare' && location === 'offboard') {...} position
                // has not changed; do nothing source piece is a spare piece and position is on
                // the board
        ...
        widget.orientation = function (arg) {
            ...
            // flip orientation
            if (arg === 'flip') {
                currentOrientation = currentOrientation === 'white'
                    ? 'black'
                    : 'white'
                drawBoard()
                return currentOrientation
        ...
        widget.resize = function () {
            ...
            // set drag piece size
            $draggedPiece.css({height: squareSize, width: squareSize})
        ...
        function initDOM() {
            ...
            // TODO: need to remove this dragged piece element if the board is no longer in
            // the DOM get the border size
