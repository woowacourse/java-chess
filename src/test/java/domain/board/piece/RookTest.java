...
class RookTest {
    ...
    Stream<DynamicTest> testIsMovable() {
        final Piece rook = Piece.rookBelongs(Camp.BLACK);
        return Stream.of(
    ...
    void sameCampPieceExistInDestination() {
        ...
        final Location start = Location.of(Row.valueOf(1), Column.valueOf(1));
        final Location end = Location.of(Row.valueOf(1), Column.valueOf(2));
        Path path = new Path(new PieceMove(start, end), List.of(rook, Piece.pawnBelongs(Camp.BLACK)));
        assertThatThrownBy(() -> rook.validatePath(path))
            .isInstanceOf(IllegalArgumentException.class);
    }
    ...
    void pathIsBlocked() {
        ...
        Path path = new Path(new PieceMove(start, end), List.of(rook, Piece.pawnBelongs(Camp.BLACK), Piece.empty()));
        assertThatThrownBy(() -> rook.validatePath(path))
            .isInstanceOf(IllegalArgumentException.class);
    }
}
