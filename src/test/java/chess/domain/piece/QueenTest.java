...
class QueenTest {
    ...
    @ParameterizedTest
    @DisplayName("퀸이 움직이는 방향을 확인한다")
    @MethodSource
    void isMovableDirection(Direction direction, boolean result) {
        Queen queen = new Queen(WHITE);
        Assertions.assertThat(queen.isMovableDirection(direction)).isEqualTo(result);
    }
    ...
    @ParameterizedTest
    @DisplayName("퀸이 움직이는 거리를 확인한다")
    @MethodSource
    void isMovableDistance(LocationDiff locationDiff, boolean result) {
        Queen queen = new Queen(WHITE);
        Assertions.assertThat(queen.isMovableDistance(locationDiff)).isEqualTo(result);
    }
