...
class InitialPawnTest {
    ...
    @ParameterizedTest(name="InitialPawn은 앞으로 2칸까지 이동할 수 있다. 상대팀이 위치하는 경우 대각선으로도 이동할 수 있다.")
    ...
    @ParameterizedTest(name="InitialPawn은 앞으로 2칸까지만 이동할 수 있다. 이 외의 움직임은 예외가 발생한다.")
    ...
    @ParameterizedTest(name="WhiteInitialPawn은 위로만, BlackInitialPawn은 아래로만 이동할 수 있다.")
    ...
    @ParameterizedTest(name="InitialPawn은 같은 팀 말이 있는 곳으로 이동할 수 없다.")
