...
class KnightTest {
    ...
    void canMove(final String input) {
        ...
        assertThat(white.canMove(source, new Position(inputs[0], inputs[1]), new King(Team.BLACK))).isTrue();
        assertThat(white.canMove(source, new Position(inputs[0], inputs[1]), Blank.getInstance())).isTrue();
        assertThat(black.canMove(source, new Position(inputs[0], inputs[1]), new King(Team.WHITE))).isTrue();
    }
}