...
class PawnTest {
	...
	void moveBlackPawnFirst(int row, int column) {
		Direction direction = blackPawn.checkMovableRange(
			new Position(7, 4),
	...
	void moveBlackPawnNotFirst(int row, int column) {
		assertThatThrownBy(() -> blackPawn.checkMovableRange(
			new Position(5, 4),
			new Position(row, column))
	...
	void moveWhitePawnFirst(int row, int column) {
		Direction direction = whitePawn.checkMovableRange(
			new Position(2, 4),
			new Position(row, column));
	...
	void moveWhitePawnNotFirst(int row, int column) {
		assertThatThrownBy(() -> whitePawn.checkMovableRange(
			new Position(3, 4),
			new Position(row, column))
		).isInstanceOf(IllegalArgumentException.class);
	}
	...
	void moveWhitePawnDiagonal (int row, int column, String result) {
		assertThat(whitePawn.checkMovableRange(
			new Position(2, 4),
