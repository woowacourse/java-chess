...
public class PawnTest {
	...
	void createMovablePositions_normal_test(String input) {
		...
		assertThat(pawn.createMovablePositions(pieces.getPieces()).getPositions()).contains(PositionFactory.of(input));
	}
	...
	void createMovablePositions_initial_all_empty_test() {
		...
		assertThat(pawn.createMovablePositions(Collections.emptyList()).getPositions().size()).isEqualTo(2);
	}
	...
	void createMovablePositions_not_initial_all_empty_test() {
		...
		assertThat(pawn.createMovablePositions(Collections.emptyList()).getPositions().size()).isEqualTo(1);
	}
	...
	void createMovablePositions_initial_blocked_test() {
		...
		assertThat(pawn.createMovablePositions(pieces.getPieces()).getPositions().size()).isEqualTo(0);
	}
}
