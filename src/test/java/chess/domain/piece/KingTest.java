...
public class KingTest {
    ...
    void 이동_가능한_경우_예외를_던지지_않는다(Position targetPosition) {
        ...
        setPiece(chessBoard, SOURCE_POSITION, king);

        assertDoesNotThrow(
                () -> king.validateMove(chessBoard.getBoard(), new Position(SOURCE_POSITION), targetPosition));
    }
    ...
    void 이동_가능하고_같은진영의_기물이_위치한_경우_예외를_던진다(String target) {
        ...
        assertThatThrownBy(
                () -> king.validateMove(chessBoard.getBoard(), new Position(SOURCE_POSITION), new Position(target)))
                .isInstanceOf(IllegalArgumentException.class);
    }
}
