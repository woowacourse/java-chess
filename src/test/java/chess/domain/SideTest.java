...
class SideTest {
	...
	private static Stream<Arguments> providePawn() {
		return Stream.of(
				Arguments.of(Side.WHITE, Position.of("a2"), true),
				Arguments.of(Side.BLACK, Position.of("d7"), true));
	}
	...
	void isAttackForwardWhite(String source, String target, boolean expected) {
		assertThat(Side.WHITE.isAttackForward(Position.of(source), Position.of(target))).isEqualTo(expected);
	}
	...
	void isAttackForwardBlack(String source, String target, boolean expected) {
		assertThat(Side.BLACK.isAttackForward(Position.of(source), Position.of(target))).isEqualTo(expected);
	}
