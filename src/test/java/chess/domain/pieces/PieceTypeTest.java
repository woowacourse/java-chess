...
class PieceTypeTest {
    ...
    void Piece를_받아_PieceType을_반환한다() {
        assertAll(
                () -> assertThat(PieceType.from(new BlackPawn())).isEqualTo(PAWN),
                () -> assertThat(PieceType.from(new Rook(WHITE))).isEqualTo(ROOK),
                () -> assertThat(PieceType.from(new Knight(WHITE))).isEqualTo(KNIGHT),
                () -> assertThat(PieceType.from(new Bishop(WHITE))).isEqualTo(BISHOP),
                () -> assertThat(PieceType.from(new Queen(WHITE))).isEqualTo(QUEEN),
                () -> assertThat(PieceType.from(new King(WHITE))).isEqualTo(KING),
                () -> assertThat(PieceType.from(new EmptyPiece())).isEqualTo(EMPTY)
        );
    }
    ...
    void 하나의_열에_있는_기물들의_총점을_계산한다() {
        ...
        assertThat(PieceType.calculateScore(pieces)).isEqualTo(new Score(30.0));
    }
    ...
    void 하나의_열에_폰이_하나면_1점으로_계산한다() {
        ...
        assertThat(PieceType.calculateScore(pieces)).isEqualTo(new Score(1));
    }
    ...
    void 하나의_열에_폰이_2개_이상이면_각_0_5점으로_계산한다_case2() {
        ...
        assertThat(PieceType.calculateScore(pieces)).isEqualTo(new Score(1.5));
    }

    @Test
    @DisplayName("Piece가 없으면 0점을 반환한다")
    void Piece가_없으면_0점을_반환한다() {
        Score score = PieceType.calculateScore(List.of());
