...
public class ChessGameTest {
    ...
    @Test
    @DisplayName("move가 정상적으로 되었는지 확인한다(폰 시작 시 두칸)")
    void move_pawn() {
        Map<Square, Piece> board = createBlankBoard();
        board.put(new Square("c3"), WHITE_PAWN);
        Board chessBoard = new Board(board);
        ChessGame chessGame = new ChessGame(chessBoard, Color.WHITE);
        chessGame.move(new Square("c3"), new Square("c5"));

        assertThat(chessBoard.getBoard().get(new Square("c5")))
