...
class BishopTest {
    ...
    private final Piece blackBishop = new Piece(PieceType.BISHOP, Camp.BLACK);

    @Test
    @DisplayName("pick()은 호출하면 자기 자신을 반환한다")
    void pick_whenCall_thenReturnThis() {
        // when
        final Piece actual = blackBishop.pick();

        // then
        assertThat(actual).isSameAs(blackBishop);
    }
    ...
    void isNotPassable_whenCall_thenReturnFalse() {
        // when
        final boolean actual = blackBishop.isNotPassable();
    ...
    void isSameTeam_givenCamp_thenReturnIsSameTeam(final Camp camp, final boolean expected) {
        // when
        final boolean actual = blackBishop.isSameTeam(camp);
    ...
    class BlackBishopMovableMethodTest {
        ...
        void movable_givenValidDistanceAndEnemyTarget_thenReturnTrue(final int file, final int rank) {
            ...
            // when
            final boolean actual = blackBishop.movable(distance, enemy);
        ...
        void movable_givenValidDistanceAndAllyTarget_thenReturnFalse() {
            ...
            // when
            final boolean actual = blackBishop.movable(distance, ally);
        ...
        void movable_giveInvalidDistance_thenReturnFalse(final int invalidFile, final int invalidRank) {
            ...
            // when
            final boolean actual = blackBishop.movable(invalidDistance, empty);
