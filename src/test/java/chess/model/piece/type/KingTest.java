...
class KingTest {
    ...
    private final Piece blackKing = new Piece(PieceType.KING, Camp.BLACK);
    
    @Test
    @DisplayName("pick()은 호출하면 자기 자신을 반환한다")
    void pick_whenCall_thenReturnThis() {
        // when
        final Piece actual = blackKing.pick();

        // then
        assertThat(actual).isSameAs(blackKing);
    }
    ...
    void isNotPassable_whenCall_thenReturnFalse() {
        // when
        final boolean actual = blackKing.isNotPassable();
    ...
    void isSameTeam_givenCamp_thenReturnIsSameTeam(final Camp camp, final boolean expected) {
        // when
        final boolean actual = blackKing.isSameTeam(camp);
    ...
    class KingMovableMethodTest {
        private final Piece ally = new Piece(PieceType.BISHOP, Camp.BLACK);
        ...
        void movable_givenValidDistanceAndEnemyTarget_thenReturnTrue(final int file, final int rank) {
            ...
            // when
            final boolean actual = blackKing.movable(distance, enemy);
        ...
        void movable_givenValidDistanceAndAllyTarget_thenReturnFalse() {
            ...
            // when
            final boolean actual = blackKing.movable(distance, ally);
        ...
        void movable_giveInvalidDistance_thenReturnFalse(final int invalidFile, final int invalidRank) {
            ...
            // when
            final boolean actual = blackKing.movable(invalidDistance, empty);

            // then
            assertThat(actual).isFalse();
        }

        @ParameterizedTest(name = "움직이는 거리가 (1, 1)보다 크다면 목적지와 무관하게 움직일 수 없다.")
        @DisplayName("movable() 유효한 이동 방향, 유효하지 않은 이동 거리 테스트")
        @CsvSource(value = {"2:2", "2:-2", "-2:-2", "-2:2", "0:2", "0:-2", "2:0", "-2:0"}, delimiter = ':')
