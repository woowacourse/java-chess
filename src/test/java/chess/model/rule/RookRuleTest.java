...
class RookRuleTest {
    ...
    void 같은편에_길막당했을때() {
        ...
        board.setPiece(new Square(Column.D, Row._3), piece);

        destination = new Square(Column.D, Row._3);
        result = Rule.isValidMove(board, checkTarget, destination);
        assertThat(result).isFalse();

        destination = new Square(Column.D, Row._2);
        result = Rule.isValidMove(board, checkTarget, destination);
        assertThat(result).isFalse();
